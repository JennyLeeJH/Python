...
import re
custlist = cf.data_load('python_basic/01_basic/custlist.json')
#custlist = []
page = len(custlist) - 1
...
while True:
    ...
    if choice == "I":  
       page = cf.customer_input(custlist) # page로 변경된 리턴값 받아줘야 한다.
    elif choice =="C": 
        cf.cur_page(custlist, page)
    elif choice == 'P':
        page = cf.pre_page(custlist, page)
    elif choice == 'N':
        page = cf.next_page(custlist, page)
    elif choice == 'D':
        cf.customer_delete(custlist)
    elif choice == "U": 
        cf.customer_update(custlist)
    elif choice == "Q":
        cf.data_save('python_basic/01_basic/custlist.json',custlist)
        break
