...
class TypingGame():
    ...
    def game(self): #word,rank 내가 갖고 있기 때문에 전달할 필요 없다.
        print('게임시작!')
        start = time.time()
        n = 1
...
>>> '''
    ...
    def exe(self,menu):
        if menu == '1':
            ...
            print('프로그램 종료!')
            self.data_save('python_basic/01_basic/word.json', self.word) #데이터가 상황에 따라 달라지기에 word와 rank는 받아야 한다.
    ...
    def __init__(self):
        ...
        # word와 rank 호출 할 때 self.을 꼭 붙여줘야 한다.
